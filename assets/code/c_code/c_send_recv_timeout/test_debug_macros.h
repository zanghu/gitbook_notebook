...
#define MUTEX_UNLOCK(p_lock) \
    ...
    CHK_ERR(pthread_mutex_unlock((p_lock)), ACC749_BASE_FUNC_FAILURE); \
...
// 五、其他=========================
#ifdef _SHOW_CALL_LEVEL
  #define RAISE_ERROR(code) _raise_error_with_call_level_test(code, __FUNCTION__, __FILE__, __LINE__)
#else
  #define RAISE_ERROR(code) _raise_error_without_call_level_test(code, __FUNCTION__, __FILE__, __LINE__)
#endif
