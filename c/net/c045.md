...
翻译过来大意是：
```
connect被信号中断后，系统仍会在后台异步的为之前connect调用的socket继续建立连接。

因此，如果在connect返回EINTR之后，如果对socket重启connect系统调用，有以下两种可能：
（1）如果当前socket连接尚未建立完成，那么connect会立即返回错误，errno是EINPROGRESS；
（2）如果当前socket连接已经建立完成，那么connect会立即返回错误，errno是EALREADY。
```