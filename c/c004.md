...
#include "test_utilities_so.h"

#include "load_air.h"
#include "B3_SC005360_1_RMC55_so.h"

#define _TEST_LOOP_NUM (1)

// 动态库路径
static const char * m_acc_top_dir = "../../../build/lib_so";
...
int testAccSo()
{
    ...
    setenv("AIR_CFG_DIR", "./", 1);

    // 1.加载动态库
    error = loadAirSo(m_acc_top_dir);
    if (error != 0) {
        fprintf(stderr, "loadAirSo failed, error.\n");
        return TEST_ERROR;
    }
    ...
    // 2.1.打开日志模块
    error = initCommonLoggerSo();
    if (error != 0) {
        fprintf(stderr, "日志模块打开失败, error.\n");
        return TEST_ERROR;
    }
    else {
        fprintf(stdout, "日志模块已打开\n");
    }

    // 2.2.执行测试用例
#ifdef _TEST_LOOP_NUM
    int cnt = 0;
    while (cnt < _TEST_LOOP_NUM) {
#endif


    error = test_B3_SC005360_1_RMC55_so();
    if (error != 0) {
        fprintf(stderr, "test_B3_DPMDO043_so failed, error.\n");
        return TEST_ERROR;
    }

#ifdef _TEST_LOOP_NUM
    fprintf(stdout, " 已完成第 %d 轮循环测试.\n", cnt);
    ++cnt;
    }
#endif

    // 2.3.关闭日志
    error = releaseCommonLoggerSo(); // releaseCommonLogger()
    if (error != 0) {
        fprintf(stderr, "日志模块关闭失败, error.\n");
        return TEST_ERROR;
    }
    else {
        fprintf(stdout, "日志模块已关闭.\n");
    }

    // 4.关闭动态库
    error = unloadAirSo();
    if (error != 0) {
        fprintf(stderr, "AirSo关闭失败, error.\n");
        return TEST_ERROR;
    }
