...
2.3节中描述的受限张量元素置换，索引张量的每个元素的索引号(y1, y2, ..., yN)有两个作用：

首先，指明了它来描述的映射是新张量中哪个元素的；

其次，索引号向量除了yk之外的N-1个分量实际上限制了新张量元素在原张量中选择元素的N-1个轴向的自由度（原本有N个轴向的自由度）。

而索引张量元素的值index\[a1\]\[a2\]...\[aN\]则给出了唯一一个自由度轴向（2.3中的轴向k）上的索引号，由此，新张量的元素对应的原张量的元素的N个索引分量全部确定，元素的置换映射也就唯一确定。
